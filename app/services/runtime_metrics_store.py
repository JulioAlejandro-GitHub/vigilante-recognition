...
DEFAULT_RUNTIME_METRICS_FILENAME = "events.jsonl"


class JsonlRuntimeMetricsStore:
    """Append-only local JSONL store with simple size rotation."""

    def __init__(
        self,
        path: str | Path,
        *,
        rotate_max_mb: float = 25.0,
        retention_files: int = 5,
    ) -> None:
        self.path = resolve_runtime_metrics_path(path)
                ...
                with path.open("r", encoding="utf-8") as handle:
                    for line_number, raw_line in enumerate(handle, start=1):
                        ...
                        try:
                            ...
                            logger.warning(
                                "runtime_metrics_invalid_jsonl path=%s line=%s",
                                path,
                                line_number,
                            )
                            continue
...
def resolve_runtime_metrics_path(path: str | Path) -> Path:
    resolved = Path(path)
    if resolved.suffix.lower() in {".jsonl", ".ndjson", ".log"}:
        return resolved
    return resolved / DEFAULT_RUNTIME_METRICS_FILENAME
